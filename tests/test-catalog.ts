...
const GAP_LIST_CAP =
  'List view is capped at the first 10 rows (it reads "Showing 1-10 of N") but exposes no pagination controls and no search box, so records beyond the first 10 are unreachable. A newly created record sorts to the end of the list and therefore cannot be viewed, edited, or deactivated through the UI.';
...
export const TEST_CASES: TestCase[] = [
  ...
  { id: 'IA-FM01-F03-TC01', row: 27, section: 'ROLE AND PERMISSION ASSIGNMENT', title: 'Create New Role', expected: 'Role created successfully', mode: 'write', checks: "Created a throwaway role via Add Role and confirmed it appears in the roles table (cleaned up by deactivation afterwards).", failReason: GAP_LIST_CAP },
  ...
  { id: 'IA-FM01-F03-TC04', row: 30, section: 'ROLE AND PERMISSION ASSIGNMENT', title: 'Edit Role Details', expected: 'Changes saved successfully', mode: 'write', checks: "Renamed the role then reverted it; the updated name appeared in the table, confirming edits persist.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F03-TC05', row: 31, section: 'ROLE AND PERMISSION ASSIGNMENT', title: 'View Role Details', expected: 'All permissions and metadata visible', mode: 'readonly', checks: "Opened the role editor and confirmed the Role Name and Active fields are displayed.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F03-TC06', row: 32, section: 'ROLE AND PERMISSION ASSIGNMENT', title: 'Deactivate Role', expected: 'Role marked inactive', mode: 'write', checks: "Unchecked Active and saved; re-opening the role confirmed it now reads inactive.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F03-TC07', row: 33, section: 'ROLE AND PERMISSION ASSIGNMENT', title: 'Reactivate Role', expected: 'Role restored', mode: 'write', checks: "Re-checked Active and saved; re-opening the role confirmed it now reads active again.", failReason: GAP_LIST_CAP },
  ...
  { id: 'IA-FM01-F10-TC01', row: 47, section: 'LOCATION AND LOCATION LEVEL', title: 'Create location level (no parent)', expected: 'Level created successfully', mode: 'write', checks: "Created a location level with no parent via Add Level and confirmed it appears in the list (cleaned up by deactivation afterwards).", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F10-TC02', row: 48, section: 'LOCATION AND LOCATION LEVEL', title: 'Create Child Location Level', expected: 'Location level created and linked to parent', mode: 'write', checks: "Created a child location level linked to parent ID 1 and confirmed it appears in the list.", failReason: GAP_LIST_CAP },
  ...
  { id: 'IA-FM01-F10-TC05', row: 51, section: 'LOCATION AND LOCATION LEVEL', title: 'Deactivate Location Level', expected: 'Level marked inactive', mode: 'write', checks: "Deactivated the temp level via the Active toggle and confirmed it reads unchecked on re-open.", failReason: GAP_LIST_CAP },
  ...
  { id: 'IA-FM01-F10-TC08', row: 54, section: 'LOCATION AND LOCATION LEVEL', title: 'Visibility of Inactive Levels', expected: 'Inactive not visible', mode: 'write', failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F10-TC09', row: 55, section: 'LOCATION AND LOCATION LEVEL', title: 'Create Parent Location (no parent)', expected: 'Location created', mode: 'write', checks: "Created a parent location with no parent and confirmed it appears in the locations table.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F10-TC10', row: 56, section: 'LOCATION AND LOCATION LEVEL', title: 'Create child location', expected: 'Location created and linked to parent', mode: 'write', checks: "Created a child location linked to parent ID 1 and confirmed it appears in the table.", failReason: GAP_LIST_CAP },
  ...
  { id: 'IA-FM01-F10-TC14', row: 60, section: 'LOCATION AND LOCATION LEVEL', title: 'Change Parent Location', expected: 'Parent updated', mode: 'write', checks: "Set the location's Parent ID then reverted it; the update saved without error.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F10-TC15', row: 61, section: 'LOCATION AND LOCATION LEVEL', title: 'Deactivate Location', expected: 'Location inactive', mode: 'write', checks: "Deactivated the temp location via the Active toggle and saved.", failReason: GAP_LIST_CAP },
  { id: 'IA-FM01-F10-TC16', row: 62, section: 'LOCATION AND LOCATION LEVEL', title: 'Activate Location', expected: 'Location active', mode: 'write', checks: "Reactivated the temp location and confirmed the Active toggle reads checked on re-open.", failReason: GAP_LIST_CAP },
